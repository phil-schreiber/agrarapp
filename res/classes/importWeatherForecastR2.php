...
class importWeatherForecast{
	...
	function testWeatherDataFile(){
		...
		$file = '/var/www/t/fileadmin/files2/importWeatherDataFile.txt';
	...
	function testRainMapFile(){
		...
		//Datei mit den Infos zum letzten Import-Zyklus auslesen
		$file = '/var/www/t/fileadmin/files2/importWeatherRainFile.txt';
		//$lastImportArray = json_decode(file_get_contents($file),1);
		//Wenn die Datei leer ist, dann default-Wert nutzen
		if(!isset($lastImportArray['importTime'])){
			$lastImportArray['importTime'] = 0;
	...
	function importMiscMaps(){
		...
		foreach($filesNew AS $key => $value){
			...
			if(!is_dir('/var/www/t/fileadmin/files2/weather/maps/')){
				mkdir('/var/www/t/fileadmin/files2/weather/maps/', 0750, true);
				chown('/var/www/t/fileadmin/files2/weather/maps/','www-data');
			}
			//Sollte das Verzeichnis für den Kartentyp nicht bestehen, dann anlegen und für Server schreibbar machen
			if(!is_dir('/var/www/t/fileadmin/files2/weather/maps/'. $mapType  .'/')){
				mkdir('/var/www/t/fileadmin/files2/weather/maps/'. $mapType  .'/' , 0750, true);
				chown('/var/www/t/fileadmin/files2/weather/maps/'. $mapType  .'/','www-data');
			}

			//Ãœberprüfung, wann der letzte Import-Zyklus für diesen Kartentyp stattgefunden hat
			$file = '/var/www/t/fileadmin/files2/importWeatherMaps'. $key  .'.txt';
			$lastImportArray = json_decode(file_get_contents($file),1);
			...
			foreach($value AS $key1 => $value1){
				...
				if(!is_dir('/var/www/t/fileadmin/files2/weather/maps/'. $mapType .'/'. $imageRegionFinal .'/')){
					mkdir('/var/www/t/fileadmin/files2/weather/maps/'. $mapType  .'/'. $imageRegionFinal .'/', 0750, true);
					chown('/var/www/t/fileadmin/files2/weather/maps/'. $mapType  .'/'. $imageRegionFinal .'/','www-data');
				}
				...
				$destinationFolder = '/var/www/t/fileadmin/files2/weather/maps/'. $mapType .'/'. $imageRegionFinal;
	...
	function importWeatherData(){
		...
		if ($res === TRUE) {
			...
			$zip->extractTo('/var/www/t/fileadmin/files2/import_weather/');
			$zip->close();
			$this->importForecast();
		}
	}
	...
	function importRainMaps(){
		...
		if ($res === TRUE) {
			...
			$zip->extractTo('/var/www/t/fileadmin/files2/import_rainmaps/');
			$zip->close();
	...
	function importRainMapFiles(){
		...
		$folderPath = '/var/www/t/fileadmin/files2/import_rainmaps';
		$dh  = opendir($folderPath);
		...
		foreach($filesNew AS $imageRegion => $imageFiles){
			...
			if(!is_dir('/var/www/t/fileadmin/files2/weather/maps/RAIN/')){
				mkdir('/var/www/t/fileadmin/files2/weather/maps/RAIN/' , 0750, true);
				chown('/var/www/t/fileadmin/files2/weather/maps/RAIN/','www-data');
			}
			if(!is_dir('/var/www/t/fileadmin/files2/weather/maps/RAIN/'. $mapArray[$imageRegion].'/')){
				mkdir('/var/www/t/fileadmin/files2/weather/maps/RAIN/'. $mapArray[$imageRegion] .'/', 0750, true);
				chown('/var/www/t/fileadmin/files2/weather/maps/RAIN/'. $mapArray[$imageRegion] .'/','www-data');
			}

			//Reset des Counters
			$i=0;
			//Ã–ffnen des Verzeichnis für die aktuelle Karten-Region
			$dh  = opendir('/var/www/t/fileadmin/files2/weather/maps/RAIN/'. $mapArray[$imageRegion]);
			//Definition des Zielordners für die nachfolgende Bereinigungsfunktion
			$destinationFolder = '/var/www/t/fileadmin/files2/weather/maps/RAIN/'. $mapArray[$imageRegion];
			while (false !== ($filename = readdir($dh))) {
				if(is_file($destinationFolder.'/'.$filename)){
			...
			foreach($imageFiles AS $key => $value){
				...
				$filePath = '/var/www/t/fileadmin/files2/import_rainmaps/'. $value;
	...
	function importForecast(){
		...
		$sourceFile = '/var/www/t/fileadmin/files2/import_weather/stadtprognose_plz_d.xml';
		//Parsen der Datei
	...
	function parseWeatherData($array){
		...
		$file = '/var/www/t/fileadmin/files2/forecast/'. $array['plz'] .'.txt';
		...
		$file = '/var/www/t/fileadmin/files2/weatherlookup/'. $array['plz'] .'.txt';
